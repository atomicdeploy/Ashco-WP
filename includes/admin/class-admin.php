...
final class Admin {
    ...
    public static function download_current_catalog_report(): void {
        ...
        Current_Catalog_Csv_Exporter::prepare_request();
        $criteria = array(
        ...
        $exporter = new Current_Catalog_Csv_Exporter($service);
        $result = $exporter->write($stream, $report, $criteria);
        ...
        header('Content-Length: ' . (int) $result['bytes']);
        header('Content-Disposition: attachment; filename="ashko-current-catalog-' . gmdate('Ymd-His') . '.csv"');
        $output = fopen('php://output', 'wb');
        if (false !== $output) {
            while (!feof($stream)) {
                $chunk = fread($stream, Current_Catalog_Csv_Exporter::OUTPUT_CHUNK_BYTES);
    ...
    private static function discard_output_buffers(): void {
        while (ob_get_level() > 0 && @ob_end_clean()) {
            // Discard only PHP output buffers; the completed CSV is file-backed.
        }
    }
