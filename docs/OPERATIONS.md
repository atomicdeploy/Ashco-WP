...
5. Configure exact `{source id,dataset}` scopes and copy the generated secret into the Patris service environment.
6. Start the new receiver state with a complete snapshot. Do not begin with an update event.
7. Send the snapshot to `/dry-run` repeatedly while `retryable` is true.
8. Download/review the run CSV and warning groups.
