 ...
 *
 * Patris Export hashes decimal JSON lexemes before transport. PHP's
 ...
 */
final class Product_Sync_JSON_Number {
    /** @var string */
    public $value;
...
/** Marker that keeps an empty JSON object distinct from an empty JSON array. */
final class Product_Sync_JSON_Empty_Object {}

/**
 * Strict JSON decoder with duplicate-key rejection and exact number tokens.
 */
final class Product_Sync_JSON_Decoder {
    ...
    private function parse_object($depth) {
        ...
        if ($this->consume_character('}')) {
            return new Product_Sync_JSON_Empty_Object();
        }
    ...
    private function parse_number() {
        ...
        return new Product_Sync_JSON_Number($token);
    }
...
class Product_Sync_Receiver {
    ...
    private const LOCK_NAME = 'ashko_product_sync';
    ...
    public function receive_json($json) {
        ...
        $payload = $this->decode_json_document($json);
    ...
    public function preview_json($json) {
        ...
        if ('update' === $envelope['event_type'] && !is_array($existing)) {
            return $this->error(
                'ashko_product_sync_baseline_required',
                'An update event cannot be validated before its source snapshot.',
                409
            );
        }
        if (is_array($existing)) {
            $comparison = $this->compare_timestamp_order(
                $envelope['generated_at_order'],
                $existing['generated_at_order'] ?? array()
            );
            if ($comparison < 0) {
                return $this->error('ashko_product_sync_stale_event', 'The event is older than the stored source state.', 409);
            }
            if (0 === $comparison && $envelope['event_id'] !== ($existing['last_event_id'] ?? '')) {
                return $this->error(
                    'ashko_product_sync_order_conflict',
        ...
        return array(
            ...
            'replay' => is_array($existing) && isset($existing['recent_events'][$envelope['event_id']]),
        );
    }

    /** @return array|WP_Error */
    private function decode_json_document($json) {
        if (!is_string($json) || '' === trim($json)) {
            return $this->error('ashko_product_sync_invalid_json', 'A JSON request body is required.', 400);
        }
        if (strlen($json) > self::MAX_BODY_BYTES) {
            return $this->error('ashko_product_sync_payload_too_large', 'The product-sync payload is too large.', 413);
        }

        try {
            $payload = Product_Sync_JSON_Decoder::decode($json);
        } catch (RuntimeException $exception) {
            return $this->error(
                'ashko_product_sync_invalid_json',
    ...
    /**
     * Validate, order, persist, and apply a product-sync envelope.
     *
    ...
    public function receive($payload) {
        if (!is_array($payload) || array_is_list($payload)) {
            return $this->error('ashko_product_sync_invalid_payload', 'The product-sync payload must be an object.', 400);
        }

        $forbidden = $this->find_forbidden_raw_key($payload);
        if (null !== $forbidden) {
            return $this->error(
                'ashko_product_sync_raw_key_forbidden',
        ...
        $currency_check = $this->validate_store_currency();
        if (is_wp_error($currency_check)) {
        ...
        try {
            ...
            return $this->error(
                'ashko_product_sync_unexpected_failure',
    ...
    public function reconcile($source_id = null, $dataset = null) {
        if ((null === $source_id) !== (null === $dataset)) {
            return $this->error(
                'ashko_product_sync_reconcile_scope_invalid',
        ...
        try {
            ...
            if (null !== $source_id) {
                ...
                if (!isset($state['sources'][$source_key]) || !is_array($state['sources'][$source_key])) {
                    return $this->error(
                        'ashko_product_sync_source_not_found',
            ...
            return $this->error(
                'ashko_product_sync_reconcile_failed',
    ...
    private function receive_locked($envelope) {
        ...
        if (null === $existing && count($state['sources']) >= self::MAX_SOURCES) {
            return $this->error('ashko_product_sync_source_limit', 'The configured source limit has been reached.', 409);
        }
    ...
    private function emit_result($result, $envelope) {
        try {
            do_action('ashko_product_sync_applied', $result, array(
            ...
            $result['delivery_warnings'][] = array(
                'code' => 'ashko_product_sync_listener_failed',
        ...
        try {
            ...
            $result['delivery_warnings'][] = array(
                'code' => 'ashko_product_sync_log_failed',
                'exception' => get_class($exception),
            );
        }
    ...
    private function validate_envelope($payload) {
        ...
        if (!empty($unknown)) {
            return $this->error('ashko_product_sync_unknown_field', 'The envelope contains unsupported fields.', 422, array('fields' => $unknown));
        }
        ...
        if (!empty($missing)) {
            return $this->error('ashko_product_sync_missing_field', 'The envelope is missing required fields.', 422, array('fields' => $missing));
        }
        ...
        if (!Config::source_allowed((string) $source['id'], (string) $source['dataset'])) {
            return $this->error(
                'ashko_product_sync_source_forbidden',
                'This exact Patris source is not allowed.',
                403
            );
        }
        ...
        if (count($payload['products']) > self::MAX_PRODUCTS) {
            return $this->error('ashko_product_sync_product_limit', 'The event contains too many products.', 413);
        }
        ...
        foreach ($payload['products'] as $index => $product) {
            ...
            if (isset($seen_codes[$code])) {
                return $this->error('ashko_product_sync_duplicate_code', 'Product codes must be unique inside an event.', 422, array('product_code' => $code));
            }
        ...
        foreach ($seen_codes as $code) {
            if (isset($deleted_lookup[$code]) || isset($quarantined_lookup[$code])) {
                return $this->error('ashko_product_sync_code_overlap', 'A code cannot be both a product and a tombstone or quarantine entry.', 422, array('product_code' => $code));
            }
        }
        foreach ($deleted_lookup as $code) {
            if (isset($quarantined_lookup[$code])) {
                return $this->error('ashko_product_sync_code_overlap', 'Quarantined codes cannot be tombstoned.', 422, array('product_code' => $code));
            }
        }
        ...
        if (array_key_exists('local_currency', $payload)) {
        ...
        if (!hash_equals($expected_event_id, $envelope['event_id'])) {
            return $this->error(
                'ashko_product_sync_event_hash_mismatch',
    ...
    private function validate_product($product, $index) {
        ...
        $missing = array_values(array_diff(array('product_code', 'warnings', 'record_hash'), array_keys($product)));
        $unknown = array_values(array_diff(array_keys($product), self::PRODUCT_FIELDS));
        if (!empty($missing) || !empty($unknown)) {
            return $this->error(
                'ashko_product_sync_product_shape_invalid',
                'A product does not match the sparse product-sync shape.',
                422,
        ...
        foreach (self::PRODUCT_STRING_FIELDS as $field) {
            ...
            if (null !== $product[$field] && !is_string($product[$field])) {
                return $this->field_error($path . '.' . $field, 'must be a string or null');
            }
            if (
                null !== $product[$field]
                && in_array($field, array('category_code', 'shipping_method_id'), true)
                && (trim($product[$field]) !== $product[$field] || strlen($product[$field]) > self::MAX_CODE_LENGTH)
            ) {
                return $this->field_error($path . '.' . $field, 'must be a trimmed string within the code limit or null');
            }
        }
        if (
            ...
            return $this->field_error($path . '.foreign_currency', 'must be CNY or null');
        }
        ...
        if ($has_shipping_price !== $has_shipping_currency) {
            $missing_shipping_field = $has_shipping_price
                ? 'shipping_price_per_kg_currency'
                : 'shipping_price_per_kg';
            return $this->field_error(
                $path . '.' . $missing_shipping_field,
                'must be present whenever the paired shipping field is present'
            );
        }
        if (
            ...
            && (
                !is_string($product['shipping_price_per_kg_currency'])
                || !in_array($product['shipping_price_per_kg_currency'], array('CNY', 'IRR'), true)
            )
        ) {
            return $this->field_error($path . '.shipping_price_per_kg_currency', 'must be CNY, IRR, or null');
        }
        ...
        if (
            array_key_exists('price_rounding_digits', $product)
        ...
        if (
            ...
            return $this->field_error(
                $path . '.foreign_price',
                'must not be negative; zero remains a valid raw fact but is not a usable selected price'
            );
        }
        ...
        $shipping_policy_check = $this->validate_shipping_policy($product, $path);
        ...
        if (
            ...
            return $this->field_error($path . '.weight_grams', 'must not be negative when provided');
        }
        if (
        ...
        if (array_key_exists('warehouse_stock', $product) && null !== $product['warehouse_stock']) {
            if (
                !($product['warehouse_stock'] instanceof Product_Sync_JSON_Empty_Object)
                && (!is_array($product['warehouse_stock']) || array_is_list($product['warehouse_stock']))
            ) {
                return $this->field_error($path . '.warehouse_stock', 'must be an object or null');
            }
            if (is_array($product['warehouse_stock'])) {
                foreach ($product['warehouse_stock'] as $warehouse => $stock) {
                    if ('' === trim((string) $warehouse) || (null !== $stock && !$this->is_json_number($stock))) {
                        return $this->field_error($path . '.warehouse_stock', 'must map non-empty string keys to JSON numbers or null');
                    }
                }
            }
        }
        ...
        if (!hash_equals($expected_hash, $product['record_hash'])) {
            return $this->error(
                'ashko_product_sync_record_hash_mismatch',
        ...
        foreach ($product as $field => $value) {
            if ('warehouse_stock' === $field) {
                if (null === $value) {
                    $stored[$field] = null;
                } elseif ($value instanceof Product_Sync_JSON_Empty_Object) {
                    $stored[$field] = array();
                } else {
                    $stocks = $value;
                    ksort($stocks, SORT_STRING);
                    $stored[$field] = array_map(function($stock) {
                        return null === $stock ? null : $this->number_to_storage($stock);
                    }, $stocks);
                }
            } elseif (in_array($field, self::PRODUCT_DECIMAL_FIELDS, true)) {
                $stored[$field] = null === $value ? null : $this->decimal_to_storage($value);
            } elseif (in_array($field, self::PRODUCT_NULLABLE_NUMBER_FIELDS, true) || 'final_price' === $field) {
                $stored[$field] = null === $value ? null : $this->number_to_storage($value);
    ...
    private function validate_categories($values) {
        ...
        if (count($values) > self::MAX_CATEGORIES) {
            return $this->error('ashko_product_sync_category_limit', 'The event contains too many categories.', 413);
        }
        ...
        foreach ($values as $index => $category) {
            ...
            if (!empty($missing) || !empty($unknown)) {
                return $this->error(
                    'ashko_product_sync_category_shape_invalid',
                    'A category does not match the product-sync category shape.',
                    422,
                    array('path' => $path, 'missing' => $missing, 'unknown' => $unknown)
                );
            }
            ...
            if (isset($seen[$category['category_code']])) {
                return $this->error(
                    'ashko_product_sync_duplicate_category_code',
                    'Category codes must be unique inside an event.',
                    422,
                    array('category_code' => $category['category_code'])
                );
            }
            if ((null !== $category['name'] && !is_string($category['name'])) || !is_string($category['parent_code'])) {
                return $this->field_error($path, 'name must be a string or null and parent_code must be a string');
            }
            if (
            ...
            if (!hash_equals($expected_hash, $category['record_hash'])) {
                return $this->error(
                    'ashko_product_sync_category_hash_mismatch',
    ...
    private function validate_catalog_projection($products, $categories, $excluded_codes) {
        ...
        foreach ($excluded_codes as $code) {
            ...
            if (isset($category_lookup[$code])) {
                return $this->error(
                    'ashko_product_sync_catalog_overlap',
        ...
        foreach ($products as $product) {
            ...
            if ('' !== $category_code && !isset($category_lookup[$category_code])) {
                return $this->error(
                    'ashko_product_sync_category_reference_invalid',
    ...
    private function build_transition($envelope, $existing) {
        ...
        if (!hash_equals($expected_revision, $envelope['source']['revision'])) {
            return $this->error(
                'ashko_product_sync_source_revision_mismatch',
    ...
    private function drain_delivery_products(&$source_state, $include_pending, $include_deferred) {
        ...
        foreach ($work as $code_key => $delivery_entry) {
            if ((microtime(true) - $started_at) >= self::DELIVERY_TIME_BUDGET_SECONDS) {
            ...
            if (!$product) {
                ...
                $this->append_woo_error($result, array(
                    'product_code' => $product_code,
                    'code' => 'ashko_product_sync_woocommerce_product_unavailable',
                    'retryable' => true,
                ));
                continue;
            }
            ...
            try {
                ...
                $this->append_woo_error($result, array(
                    'product_code' => $product_code,
                    'code' => 'ashko_product_sync_woocommerce_write_failed',
                    'retryable' => true,
                ));
            }
        }
    ...
    private function terminal_resolution_reason($error_code) {
        if ('ashko_product_identifier_not_found' === $error_code) {
            return 'missing';
        }
        if ('ashko_product_identifier_ambiguous' === $error_code) {
            return 'ambiguous';
        }
    ...
    private function persist_and_read_back($state) {
        ...
        if (
            ...
            return $this->error('ashko_product_sync_storage_unavailable', 'The receiver storage service is unavailable.', 503);
        }
        if (false === $wpdb->query('START TRANSACTION')) {
            return $this->error('ashko_product_sync_transaction_unavailable', 'The receiver could not start a storage transaction.', 503);
        }
        ...
        if (!is_string($serialized) || strlen($serialized) > self::MAX_STATE_BYTES) {
            ...
            return $this->error('ashko_product_sync_state_too_large', 'The combined receiver state is too large.', 413);
        }
        ...
        if (false === $written || 0 === $written) {
            ...
            return $this->error('ashko_product_sync_storage_failed', 'The receiver state could not be stored.', 500);
        }
        ...
        if (!is_array($read_back) || !hash_equals($this->state_digest($state), $this->state_digest($read_back))) {
            ...
            return $this->error('ashko_product_sync_readback_failed', 'Stored receiver state did not pass readback verification.', 500);
        }
        if (false === $wpdb->query('COMMIT')) {
            $wpdb->query('ROLLBACK');
            $this->invalidate_state_cache();

            return $this->error('ashko_product_sync_commit_failed', 'The receiver state transaction could not be committed.', 500);
        }
    ...
    private function validate_price_source_selection($product, $path) {
        $selection_fields = array('price_source_amount', 'price_source_currency', 'price_source_kind');
        $present_selection_fields = array_values(array_filter(
            $selection_fields,
            static fn($field) => array_key_exists($field, $product)
        ));
        if (0 !== count($present_selection_fields) && count($selection_fields) !== count($present_selection_fields)) {
            return $this->error(
                'ashko_product_sync_price_source_shape_invalid',
                'The selected price source fields must be supplied together or omitted together.',
                422,
                array(
                    'path' => $path,
                    'missing' => array_values(array_diff($selection_fields, $present_selection_fields)),
                )
            );
        }

        $has_rounding_digits = array_key_exists('price_rounding_digits', $product);
        $has_rounding_mode = array_key_exists('price_rounding_mode', $product);
        $rounding_is_explicit_null = $has_rounding_digits && null === $product['price_rounding_digits'];
        if ($rounding_is_explicit_null && $has_rounding_mode) {
            return $this->field_error(
                $path . '.price_rounding_mode',
                'must be omitted when price_rounding_digits is explicitly null'
            );
        }
        if (!$rounding_is_explicit_null && $has_rounding_digits !== $has_rounding_mode) {
            return $this->error(
                'ashko_product_sync_rounding_shape_invalid',
                'Non-null rounding digits and rounding mode must be supplied together or omitted together.',
                422,
                array(
                    'path' => $path,
                    'missing' => $has_rounding_digits
                        ? array('price_rounding_mode')
                        : array('price_rounding_digits'),
                )
            );
        }
        if (
            $has_rounding_digits
            && !$rounding_is_explicit_null
            && 'nearest_half_up' !== $product['price_rounding_mode']
        ) {
            return $this->field_error(
                $path . '.price_rounding_mode',
                'must be nearest_half_up'
            );
        }

        $has_selection = count($selection_fields) === count($present_selection_fields);
        if ($has_selection) {
            if (
                ...
                return $this->field_error(
                    $path . '.price_source_amount',
                    'must be greater than zero; omit the selected price source when no usable price exists'
                );
            }
            $source_kind = $product['price_source_kind'];
            $source_currency = $product['price_source_currency'];
            if (
                !(
                    ('foreign_price' === $source_kind && 'CNY' === $source_currency)
                    || ('partner_price' === $source_kind && 'IRR' === $source_currency)
                    || (
                        'sale_price_direct' === $source_kind
                        && 'IRR' === $source_currency
                ...
                return $this->field_error(
                    $path . '.price_source_kind',
                    'must select foreign_price/CNY, partner_price/IRR, or an explicitly enabled sale_price_direct/IRR'
                );
            }
        }

        $has_positive_cny = array_key_exists('foreign_price', $product)
            && null !== $product['foreign_price']
            && $this->number_compare_zero($product['foreign_price']) > 0
            && 'CNY' === ($product['foreign_currency'] ?? null);
        $markup_parts = array_key_exists('markup_percent', $product) && null !== $product['markup_percent']
            ? $this->formula_decimal_parts($product['markup_percent'])
            : array('error' => 'missing');
        $has_calculation_markup = array_key_exists('markup_percent', $product)
            && null !== $product['markup_percent']
            && $this->number_compare_zero($product['markup_percent']) >= 0
            && !isset($markup_parts['error'])
            && $this->decimal_compare(
                $markup_parts,
                $this->formula_decimal_parts(self::MAX_MARKUP_PERCENT)
            ) <= 0;
        $has_calculation_rounding = array_key_exists('price_rounding_digits', $product)
            && null !== $product['price_rounding_digits']
            && array_key_exists('price_rounding_mode', $product)
            && 'nearest_half_up' === $product['price_rounding_mode'];
        $usable_cny = $has_positive_cny
            && array_key_exists('weight_grams', $product)
            && null !== $product['weight_grams']
            && $this->number_compare_zero($product['weight_grams']) > 0
            ...
            && Decimal_Calculator::DOMESTIC_SHIPPING_METHOD !== $product['shipping_method_id']
            ...
            && in_array($product['shipping_price_per_kg_currency'] ?? null, array('CNY', 'IRR'), true)
            && array_key_exists('irt_per_cny', $product)
            && null !== $product['irt_per_cny']
            && $this->number_compare_zero($product['irt_per_cny']) > 0
            && $has_calculation_markup
            && $has_calculation_rounding;
        $has_positive_partner = array_key_exists('partner_price_source', $product)
            && null !== $product['partner_price_source']
            && $this->number_compare_zero($product['partner_price_source']) > 0;
        $usable_partner = $has_positive_partner && $has_calculation_markup && $has_calculation_rounding;
        $has_positive_direct_sale = array_key_exists('sale_price_source', $product)
            && null !== $product['sale_price_source']
            && $this->number_compare_zero($product['sale_price_source']) > 0;
        $usable_direct = $has_positive_direct_sale
            && !$usable_partner
            && 'yes' === (string) Config::get('use_sale_price_direct_fallback', 'no');
        ...
        if ($usable_cny) {
            if (
                !$has_selection
                || 'foreign_price' !== $product['price_source_kind']
                || 'CNY' !== $product['price_source_currency']
                || !$this->decimal_values_equal($product['price_source_amount'], $product['foreign_price'])
            ) {
                return $this->field_error(
                    $path . '.price_source_amount',
                    'must select the positive CNY foreign_price with priority'
                );
            }
        } elseif ($usable_partner) {
            $valid_partner_selection = $has_selection
                && 'partner_price' === $product['price_source_kind']
                && 'IRR' === $product['price_source_currency']
                && $this->decimal_values_equal($product['price_source_amount'], $product['partner_price_source']);
            if (!$valid_partner_selection) {
                return $this->field_error(
                    $path . '.price_source_amount',
                    'must select the positive IRR partner price when the partner calculation is complete'
                );
            }
        } elseif ($usable_direct) {
            if (
                !$has_selection
                || 'sale_price_direct' !== $product['price_source_kind']
                || 'IRR' !== $product['price_source_currency']
                || !$this->decimal_values_equal($product['price_source_amount'], $product['sale_price_source'])
            ) {
                return $this->field_error(
                    $path . '.price_source_amount',
                    'must select the enabled direct IRR sale price when calculated fallbacks are unavailable'
                );
            }
        } elseif ($has_selection) {
            return $this->field_error(
                $path . '.price_source_amount',
                'must be omitted when no fallback-chain price source is usable'
            );
        }
    ...
    /**
     * Domestic purchases carry explicit zero-rate freight provenance. Foreign
     * methods continue to require a positive freight rate. The calculation
     * path never applies domestic freight to an IRR partner price.
     */
    private function validate_shipping_policy($product, $path) {
        $has_method = array_key_exists('shipping_method_id', $product);
        ...
        $method = $has_method ? $product['shipping_method_id'] : null;
        $rate = $has_rate ? $product['shipping_price_per_kg'] : null;
        $currency = $has_currency ? $product['shipping_price_per_kg_currency'] : null;

        if (Decimal_Calculator::DOMESTIC_SHIPPING_METHOD === $method) {
            if (!$has_rate || null === $rate || 0 !== $this->number_compare_zero($rate)) {
                return $this->field_error(
                    $path . '.shipping_price_per_kg',
                    'must be the explicit numeric zero for domestic shipping'
                );
            }
            if (!$has_currency || Decimal_Calculator::DOMESTIC_SHIPPING_CURRENCY !== $currency) {
                return $this->field_error(
                    $path . '.shipping_price_per_kg_currency',
                    'must be IRR for domestic shipping'
                );
            }
        } elseif ($has_rate && null !== $rate && $this->number_compare_zero($rate) <= 0) {
            return $this->field_error(
                $path . '.shipping_price_per_kg',
                'must be greater than zero for a foreign shipping method'
            );
        }

        if (!array_key_exists('price_source_kind', $product)) {
            return true;
        }
        if (in_array($product['price_source_kind'], array('partner_price', 'sale_price_direct'), true)) {
            if (
                Decimal_Calculator::DOMESTIC_SHIPPING_METHOD !== $method
                || !$has_rate
                || null === $rate
                || 0 !== $this->number_compare_zero($rate)
                || Decimal_Calculator::DOMESTIC_SHIPPING_CURRENCY !== $currency
            ) {
                return $this->field_error(
                    $path . '.shipping_method_id',
                    'domestic IRR price sources must use domestic shipping with an explicit 0 IRR/kg rate'
                );
            }
        } elseif ('foreign_price' === $product['price_source_kind']) {
            if (
                !$has_method
                || !is_string($method)
                || '' === $method
                ...
                return $this->field_error(
                    $path . '.shipping_method_id',
                    'foreign_price must use a non-domestic shipping method'
                );
            }
        }
    ...
    private function decimal_values_equal($left, $right) {
        $left_parts = $this->formula_decimal_parts($left);
        $right_parts = $this->formula_decimal_parts($right);
        return !isset($left_parts['error'])
            && !isset($right_parts['error'])
            && 0 === $this->decimal_compare($left_parts, $right_parts);
    }
    ...
    private function validate_final_price_formula($product, $path) {
        $has_selected_source = array_key_exists('price_source_amount', $product)
            && array_key_exists('price_source_currency', $product)
            && array_key_exists('price_source_kind', $product);
        if (!$has_selected_source) {
            ...
            return $this->error(
                ...
                'final_price must be absent when no usable selected price source exists.',
                422,
                array('path' => $path . '.final_price', 'missing' => array('price_source_amount', 'price_source_currency', 'price_source_kind'))
            );
        }

        $source_kind = $product['price_source_kind'];
        if ('sale_price_direct' === $source_kind) {
            foreach (array('markup_percent', 'irt_per_cny', 'price_rounding_digits', 'price_rounding_mode') as $field) {
                if (array_key_exists($field, $product)) {
                    return $this->field_error(
                        $path . '.' . $field,
                        'must be omitted for sale_price_direct because the source price is used without modification'
                    );
                }
            }
        }
        $required = array('price_source_amount');
        if ('foreign_price' === $source_kind) {
            $required = array_merge(
                $required,
                array('markup_percent', 'price_rounding_digits', 'weight_grams', 'shipping_price_per_kg', 'irt_per_cny')
            );
        } elseif ('partner_price' === $source_kind) {
            $required = array_merge($required, array('markup_percent', 'price_rounding_digits'));
        }
        $missing = array();
        $decimals = array();
        ...
        if (!empty($missing)) {
            ...
            return $this->error(
                'ashko_product_sync_final_price_mismatch',
                'final_price must be absent when the selected price-source inputs are incomplete.',
                422,
        ...
        if ('sale_price_direct' === $source_kind) {
            $base_irt = $decimals['price_source_amount'];
            $base_irt['scale'] += 1; // Direct IRR to canonical IRT, exactly.
            $rounded = $this->decimal_exact_integer($base_irt);
            if (null === $rounded) {
                return $this->field_error(
                    $path . '.price_source_amount',
                    'sale_price_direct must convert exactly from IRR to an integer IRT amount'
                );
            }
        } elseif ('foreign_price' === $source_kind) {
            $shipping = $this->decimal_multiply($decimals['weight_grams'], $decimals['shipping_price_per_kg']);
            $shipping['scale'] += 3; // grams to kilograms, exactly.
            $goods_irt = $this->decimal_multiply($decimals['price_source_amount'], $decimals['irt_per_cny']);
            if ('CNY' === $product['shipping_price_per_kg_currency']) {
                $shipping_irt = $this->decimal_multiply($shipping, $decimals['irt_per_cny']);
            } else {
                $shipping_irt = $shipping;
                $shipping_irt['scale'] += 1; // IRR to IRT, exactly.
            }
            $base_irt = $this->decimal_add($goods_irt, $shipping_irt);
        } else {
            $base_irt = $decimals['price_source_amount'];
            $base_irt['scale'] += 1; // IRR partner price to IRT, exactly.
        }
        if ('sale_price_direct' !== $source_kind) {
            ...
            $rounded = $this->decimal_round_half_up_digits($marked_up, $rounding_digits);
        }
        ...
        $actual = !array_key_exists('final_price', $product) || null === $product['final_price']
            ? null
        ...
        if (!is_int($actual) || $actual !== $expected) {
            ...
            return $this->error(
                'ashko_product_sync_final_price_mismatch',
                'final_price does not match the independently evaluated selected price source.',
                422,
                $data
            );
        }
    ...
    private function formula_decimal_parts($value) {
        if ($value instanceof Product_Sync_JSON_Number) {
            $text = $value->value;
    ...
    private function decimal_round_half_up_digits($decimal, $digits) {
        $scaled = $decimal;
        $scaled['scale'] = (int) $scaled['scale'] + (int) $digits;
        return $this->normalize_big_integer(
            $this->decimal_round_half_up_integer($scaled) . str_repeat('0', (int) $digits)
        );
    }

    private function decimal_exact_integer($decimal) {
        $digits = $this->normalize_big_integer($decimal['digits']);
        $scale = (int) $decimal['scale'];
        if ($scale <= 0) {
            return $this->normalize_big_integer($digits . str_repeat('0', -$scale));
        }

        $padded = str_pad($digits, $scale + 1, '0', STR_PAD_LEFT);
        $fraction = substr($padded, -$scale);
        if ('' !== trim($fraction, '0')) {
            return null;
        }

        return $this->normalize_big_integer(substr($padded, 0, strlen($padded) - $scale));
    }
    ...
    private function event_id($envelope) {
        ...
        if (array_key_exists('formula_id', $envelope)) {
     ...
     * maps. Product record keys and warehouse_stock keys are sorted before
     * hashing to mirror the producer's canonical object encoding.
     */
    private function encode_go_json($value, $map_fields = array(), $field = '') {
        if ($value instanceof Product_Sync_JSON_Number) {
            return $value->value;
        }
        if ($value instanceof Product_Sync_JSON_Empty_Object) {
            return '{}';
        }
        if (null === $value) {
            return 'null';
    ...
    private function is_json_number($value) {
        return $value instanceof Product_Sync_JSON_Number
            || is_int($value)
    ...
    private function number_compare_zero($value) {
        $text = $value instanceof Product_Sync_JSON_Number ? $value->value : (string) $value;
        $text = ltrim($text, '+');
    ...
    private function acquire_lock() {
        ...
        if (!is_object($wpdb) || !method_exists($wpdb, 'get_var') || !method_exists($wpdb, 'prepare')) {
            return $this->error('ashko_product_sync_lock_unavailable', 'The database lock service is unavailable.', 503);
        }
        ...
        if ('1' !== (string) $locked) {
            return $this->error('ashko_product_sync_busy', 'Another product-sync event is being applied. Please retry.', 503, array('retryable' => true));
        }
        $this->lock_depth = 1;
    ...
    private function field_error($field, $reason) {
        return $this->error(
            'ashko_product_sync_field_invalid',
    ...
    private function validate_store_currency() {
        $currency_status = WooCommerce_Currency_Status::instance()->get_status();
        ...
        return $this->error(
            ...
            array(
                ...
                'required_currency' => WooCommerce_Currency_Status::REQUIRED_CURRENCY,
                'warning' => WooCommerce_Currency_Status::INCOMPATIBLE_WARNING,
            )
        );
    }

    private function error($code, $message, $status, $details = array()) {
        return new WP_Error($code, __($message, 'ashko-wp'), array_merge(array('status' => (int) $status), $details));
    }
}
